...
class ErrandResource {
	...
	ResponseEntity<Void> patchErrandWithDecision(
		...
		return created(
			// fromPath("/{municipalityId}/decisions/{decisionId}").buildAndExpand(municipalityId, decision.getId()).toUri())
			fromPath("/decisions/{decisionId}").buildAndExpand(decision.getId()).toUri())
			.header(CONTENT_TYPE, ALL_VALUE)
			.build();
	}
	...
	ResponseEntity<Void> postErrandFacility(
		...
		return created(
			...
			fromPath("/errands/{id}/facilities/{facilityId}")
				.buildAndExpand(errandId, result.getId())
				.toUri())
			.header(CONTENT_TYPE, ALL_VALUE)
			.build();
	}
	...
	ResponseEntity<Void> patchErrandWithNote(
		...
		return created(
			// fromPath("/{municipalityId}/notes/{noteId}").buildAndExpand(municipalityId, note.getId()).toUri())
			fromPath("/notes/{noteId}").buildAndExpand(note.getId()).toUri())
			.header(CONTENT_TYPE, ALL_VALUE)
			.build();
	}
	...
	ResponseEntity<Void> patchErrandWithAppeal(
		...
		return created(
			// fromPath("/{municipalityId}/appeals/{appealId}").buildAndExpand(municipalityId, appeal.getId()).toUri())
			fromPath("/appeals/{appealId}").buildAndExpand(appeal.getId()).toUri())
			.header(CONTENT_TYPE, ALL_VALUE)
			.build();
	}
	...
	ResponseEntity<Void> patchErrandWithStakeholder(
		...
		return created(
			// fromPath("/{municipalityId}/stakeholders/{stakeholderId}").buildAndExpand(municipalityId,
			// stakeholder.getId()).toUri())
			fromPath("/stakeholders/{stakeholderId}").buildAndExpand(stakeholder.getId()).toUri())
			.header(CONTENT_TYPE, ALL_VALUE)
			.build();
	}
