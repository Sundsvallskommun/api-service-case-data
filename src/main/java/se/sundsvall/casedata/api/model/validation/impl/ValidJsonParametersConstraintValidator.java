...
public class ValidJsonParametersConstraintValidator implements ConstraintValidator<ValidJsonParameters, List<JsonParameter>> {
	...
	public boolean isValid(final List<JsonParameter> value, final ConstraintValidatorContext context) {
		...
		for (int i = 0; i < value.size(); i++) {
			...
			try {
				...
				useCustomMessageForValidation("validation failed for schema '%s'".formatted(param.getSchemaId()), i, context);
				hasErrors = true;
			}
		}
	...
	String getPathVariable(final String variableName) {
		final var requestAttributes = org.springframework.web.context.request.RequestContextHolder.getRequestAttributes();
		if (requestAttributes != null) {
			final var uriTemplateVars = requestAttributes.getAttribute(URI_TEMPLATE_VARIABLES_ATTRIBUTE, SCOPE_REQUEST);
			if (uriTemplateVars instanceof Map<?, ?> vars && vars.get(variableName) instanceof String value) {
				return value;
			}
		}
