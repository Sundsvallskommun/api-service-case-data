...
public final class EntityMapper {
	...
	public static Errand toErrand(final ErrandDTO dto) {
		final var errand = Optional.ofNullable(dto)
			.map(errand1 -> Errand.builder()
				...
				.withStakeholders(new ArrayList<>(dto.getStakeholders().stream().map(EntityMapper::toStakeholder).toList()))
	...
	public static Stakeholder toStakeholder(final StakeholderDTO dto) {
	...
	public static AttachmentDTO toAttachmentDto(final Attachment attachment) {
		return Optional.ofNullable(attachment).map(dto -> AttachmentDTO.builder()
			...
			.withCategory(Optional.ofNullable(attachment.getCategory()).map(AttachmentCategory::valueOf).orElse(null))
