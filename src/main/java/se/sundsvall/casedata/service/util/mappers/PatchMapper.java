...
public final class PatchMapper {
	...
	public static ErrandEntity patchErrand(final ErrandEntity errand, final PatchErrand patch) {
		final var existingKeys = ofNullable(errand.getExtraParameters()).orElse(emptyList()).stream().map(ExtraParameterEntity::getKey).toList();
		ofNullable(patch.getExtraParameters()).ifPresent(extraParams -> {
			if (!existingKeys.isEmpty()) {
				errand.getExtraParameters().removeIf(extraParam -> patch.getExtraParameters().stream().anyMatch(extra -> extra.getKey().equals(extraParam.getKey())));
			}
			errand.getExtraParameters().addAll(toErrandParameterEntityList(extraParams, errand));
		});
	...
	public static FacilityEntity patchFacility(final FacilityEntity facility, final Facility patch) {
		...
		ofNullable(patch.getAddress()).ifPresent(obj -> facility.setAddressEntity(toAddressEntity(obj)));
