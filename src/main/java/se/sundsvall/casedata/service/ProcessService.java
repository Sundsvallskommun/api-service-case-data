...
public class ProcessService {
	...
	public ProcessService(final CaseTypeRepository caseTypeRepository, final ParkingPermitIntegration parkingPermitIntegration, final ParatransitIntegration paratransitIntegration,
	...
	public String startProcess(final ErrandEntity errand) {
		...
		if (isProcessingDisabled(errand)) {
			LOGGER.info("Case type: {} is configured to not start a process, skipping process start", errand.getCaseType());
			return null;
		}
	...
	public void updateProcess(final ErrandEntity errand) {
		...
		if (CAMUNDA_USERS.contains(Optional.ofNullable(errand.getUpdatedByClient()).orElse(""))) {
			...
			return;
		} else if (isProcessingDisabled(errand)) {
			LOGGER.info("Case type: {} is configured to not update a process, skipping process update", errand.getCaseType());
			return;
		}
