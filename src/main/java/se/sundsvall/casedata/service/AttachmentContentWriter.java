 ...
 * attachment - the errand and decision attachment services, the message exchange sync and the email and web message
 * collectors - so that content is stored identically regardless of origin.
 *
 * <p>
 * Empty content is rejected rather than silently skipped. A row without content cannot be told apart from a genuinely
 * empty file, and with the legacy base64 column gone there is nothing left to fall back on - it would simply be served
 * as an empty download. Enforcing it here keeps the invariant "a persisted attachment always has content" in one place.
 * The callers that can legitimately meet an empty source - the collectors and the message exchange sync - check for it
 * and skip creating the attachment altogether, so this only fires as a backstop.
 */
@Component
public class AttachmentContentWriter {
	 ...
	 * empty or missing upload is rejected.
	 *
	 * <p>
	...
	private static ThrowableProblem emptyContent() {
		return Problem.valueOf(BAD_REQUEST, "The attachment content must not be empty");
	}
}
