...
class DecisionTest {
	...
	void builder() {
		...
		var law = List.of(new Law());
		var decidedBy = new Stakeholder();
		var decidedAt = now();
		var validFrom = now();
		var validTo = now();
		var attachments = List.of(new Attachment());
		var extraParameters = Map.of("key", "value");
		var created = now();
		var updated = now();

		// Act
		var bean = Decision.builder()
			.withId(id)
			.withErrand(errand)
	...
	void testNoDirtOnCreatedBean() {
		assertThat(Decision.builder().build()).hasAllNullFieldsOrPropertiesExcept("law", "attachments", "extraParameters", "version")
			.satisfies(bean -> {
				assertThat(bean.getLaw()).isEmpty();
				assertThat(bean.getAttachments()).isEmpty();
				assertThat(bean.getExtraParameters()).isEmpty();
				assertThat(bean.getVersion()).isZero();
			});
		assertThat(new Decision()).hasAllNullFieldsOrPropertiesExcept("law", "attachments", "extraParameters", "version")
			.satisfies(bean -> {
				assertThat(bean.getLaw()).isEmpty();
				assertThat(bean.getAttachments()).isEmpty();
				assertThat(bean.getExtraParameters()).isEmpty();
